...
app.whenReady().then(() => {
  ...
  protocol.registerFileProtocol('app', (request, callback) => {
    const filePath = decodeURIComponent(request.url.substring(7)); // Remove 'app://' prefix
    console.log('Protocol request:', request.url, '-> filePath:', filePath);

    // Try to resolve the file
    let resolvedPath;
    if (filePath.startsWith('/')) {
      // It's an absolute path from root, try public directory first
      resolvedPath = path.join(__dirname, 'public', filePath.substring(1));
    } else {
      resolvedPath = path.join(__dirname, filePath);
    }

    // If file doesn't exist in public, try relative to app directory
